...
public class HeatStrokePotionTankBlock extends Block {
    ...
    private static final Text DESEFFECT_TEXT = Text.literal("Heat Stroke (00:22)").formatted(DESDESCRIPTION_FORMATTING);
    ...
    public HeatStrokePotionTankBlock(Settings settings) {
        super(settings);
    }

    @Override
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        player.addStatusEffect(new StatusEffectInstance((RegistryEntry<StatusEffect>) ModEffects.HEAT_STROKE,450));
    ...
    public void onSteppedOn(World world, BlockPos pos, BlockState state, Entity entity) {
        if (entity instanceof LivingEntity livingEntity){
            livingEntity.addStatusEffect(new StatusEffectInstance((RegistryEntry<StatusEffect>) ModEffects.HEAT_STROKE,450));
        }
